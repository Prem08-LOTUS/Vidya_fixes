...
impl GrblStatusBuffer {
    ...
    pub fn push(&mut self, new_data: &str) -> Option<f64> {
        // [SAFETY] We accept &str here for compatibility with the current call site,
        // but we treat it as bytes immediately.
        // In a pure driver, this signature would take &[u8].
...
mod tests {
    ...
    fn test_fragmented_packet() {
        ...
        // Part 1: No newline
        let z = buffer.push("<Idle|MPos:10.0,20.0,");
        assert!(z.is_none());

        // Part 2: Completion
        let z = buffer.push("30.0|FS:0>\n");
    ...
    fn test_garbage_resilience() {
        ...
        unsafe {
            ...
            let s = std::str::from_utf8_unchecked(bytes); // Force bad string for API test
            let z = buffer.push(s);
    ...
    fn test_lifo_freshness() {
        ...
        let z = buffer.push("<Idle|MPos:0,0,10.0|>\n<Idle|MPos:0,0,20.0|>\n");
