...
import { AlertTriangle, Power, CheckCircle, RotateCcw, Anchor, Loader2 } from 'lucide-react';
...
interface Props {
  onRecover: (action: 'RESUME' | 'REHOME' | 'ABORT') => Promise<void>;
...
export const RecoveryScreen: React.FC<Props> = ({ onRecover, onVisibilityChange }) => {
  ...
  const handleAction = async (action: 'RESUME' | 'REHOME' | 'ABORT') => {
    setIsRecovering(true);
    try {
      await onRecover(action);
      // Assume success (or handled error in parent), hide the screen
      setState({ type: 'FreshStart' });
      ...
      <div className="bg-zinc-900 border border-zinc-700 max-w-3xl w-full rounded-xl shadow-2xl overflow-hidden">
        ...
        <div className="p-8 space-y-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 pt-4">
            ...
            <button
              onClick={() => handleAction('REHOME')}
              disabled={isRecovering}
              className={`group flex flex-col items-center justify-center p-6 bg-zinc-800 hover:bg-cyan-950/30 border border-zinc-700 hover:border-cyan-500/50 rounded-lg transition-all ${isRecovering ? 'opacity-50 cursor-wait' : ''}`}
            >
              {isRecovering ? <Loader2 className="w-8 h-8 text-cyan-500 mb-3 animate-spin" /> : <Anchor className="w-8 h-8 text-cyan-500 mb-3" />}
            ...
            <button
              ...
              disabled={isCritical || isRecovering}
            ...
            >
              {isRecovering ? <Loader2 className={`w-8 h-8 mb-3 ${isCritical ? 'text-zinc-600' : 'text-emerald-500'} animate-spin`} /> : <RotateCcw className={`w-8 h-8 mb-3 ${isCritical ? 'text-zinc-600' : 'text-emerald-500'}`} />}
            ...
            <button
              ...
              disabled={isRecovering}
              className={`flex flex-col items-center justify-center p-6 bg-zinc-800 hover:bg-red-950/30 border border-zinc-700 hover:border-red-500/50 rounded-lg transition-all ${isRecovering ? 'opacity-50 cursor-wait' : ''}`}
            >
              {isRecovering ? <Loader2 className="w-8 h-8 text-red-500 mb-3 animate-spin" /> : <Power className="w-8 h-8 text-red-500 mb-3" />}
